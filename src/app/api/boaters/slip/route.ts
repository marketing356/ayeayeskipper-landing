...
// GET /api/boaters/slip?email=...
// Returns the boater's active slip, vessel, and lease info across all marinas
export async function GET(req: NextRequest) {
  ...
  if (!contact?.auth_user_id) return NextResponse.json({ slip: null })

  const res = await fetch(`${ENGINE_URL}/api/v1/boater/slip`, {
    headers: {
  ...
  if (!res.ok) {
    if (res.status === 404) return NextResponse.json({ slip: null })
